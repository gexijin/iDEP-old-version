...
output$RsessionInfo <- renderUI({
	...
	i ="<h3>User settings for your record</h3>"
	  
	i = c(i,paste("Analysis were conducted using the awesome ",iDEPversion, 
		", hosted on http://ge-lab.org.",sep="") )
	i = c(i, paste("Analysis Time: ", date() ))
	...
	i = c(i, paste("Species:",converted()$species[2]))	
	...
	if(input$dataFileFormat == 2 )  {  # if FPKM, microarray
		...
		i = c(i,"vPre-processing and exploratory data analysis settings", paste("Low filter: lowFilter=", input$lowFilter) ) 
		if(input$transform == TRUE | readData()$mean.kurtosis > kurtosis.log) 
			...
			i = c(i,"<br><strong>Pre-processing settings</strong>", paste("Min. counts: minCounts=", input$minCounts) ) 
	...
	i =c(i,"<h3>R and related packages </h3>")
